...
class TrendDiscovery:
    ...
    def discover_weekly_trends(
        ...
        for category in categories:
            ...
            # Check if category is predefined or custom user input
            if category in self.TREND_CATEGORIES:
                # Use predefined queries for known categories
                queries = self.TREND_CATEGORIES[category][:2]  # Limit to 2 queries
            else:
                # For custom user input, use the category text directly as the search query
                queries = [category]
            ...
            # Randomize order slightly on force refresh for variety
            if force_refresh and len(category_trends) > max_results_per_category:
                import random
                # Keep top results but shuffle to show variety
                top_trends = category_trends[:max_results_per_category * 2]
                random.shuffle(top_trends)
                category_trends = top_trends
    ...
    def _search_trends(
        ...
        try:
            ...
            # Search Tavily with enhanced parameters for diversity
            import random

            # Add temporal variation to query for more diverse results
            time_variations = [
                f"{query} latest",
            ...
            # Randomly select query variation on force refresh
            search_query = random.choice(time_variations) if force_refresh else query

            # Vary search parameters for more diversity
            search_params = {
                "query": search_query,
                "search_depth": "advanced",
                "topic": "news",
                "max_results": min(max_results * 2, 10),  # Get more results but stay within limits
                "include_images": True,  # Include images for richer content
                "include_answer": False,  # Don't need AI-generated answers
            }

            # Use days parameter for time-based filtering (number of days for news topic)
            if max_age_days <= 7:
                search_params["days"] = 7
            elif max_age_days <= 30:
                search_params["days"] = 30
            else:
                search_params["days"] = max_age_days

            logger.info(f"Tavily search: '{search_query}' with depth={search_params['search_depth']}, max_results={search_params['max_results']}, days={search_params.get('days')}")
    ...
    def get_best_trends_for_user(
        ...
        all_trends_by_category = self.discover_weekly_trends()
