...
def trending_topics_tab():
    ...
    # Display posting results if they exist (after successful post)
    if st.session_state.get('show_posting_results', False):
        posting_results = st.session_state.get('posting_results', [])
        ...
        if posting_results:
            ...
            for platform, success, result in posting_results:
                if success:
                    if platform == "Twitter":
                        st.success(f"✅ **Posted to {platform}!**\n\n🔗 [View Tweet](https://twitter.com/user/status/{result})")
                    else:
                        st.success(f"✅ **Posted to {platform}!**\n\nPost ID: `{result}`")
                else:
            ...
            # Add button to dismiss the results
            if st.button("✅ Dismiss Results", use_container_width=True):
                st.session_state['show_posting_results'] = False
                st.session_state['posting_results'] = []
                st.rerun()

            st.markdown("---")

    # Category selector - Free text input for custom topics
    col1, col2 = st.columns([2, 1])

    with col1:
        # Initialize session state for custom categories
        if 'custom_trend_topics' not in st.session_state:
            st.session_state.custom_trend_topics = "AI research, PhD job opportunities"

        custom_topics_input = st.text_area(
            "Enter Topics to Search (comma-separated)",
            value=st.session_state.custom_trend_topics,
            placeholder="e.g., RAG systems, Claude AI, PhD positions in AI, academic research trends",
            help="Type any keywords or topics you want to find trends for. Separate multiple topics with commas.",
            height=80
        )

        # Update session state
        st.session_state.custom_trend_topics = custom_topics_input
    ...
    # Parse custom topics
    if not custom_topics_input or not custom_topics_input.strip():
        st.warning("Please enter at least one topic to search for.")
        return

    categories = [topic.strip() for topic in custom_topics_input.split(',') if topic.strip()]

    if not categories:
        st.warning("Please enter at least one valid topic.")
        return
...
def generate_and_post_from_trend(trend: Dict, user: Dict, post_to_twitter: bool = True, post_to_linkedin: bool = True):
    ...
    try:
        ...
        # Store posting results in session state to persist after rerun
        st.session_state['posting_results'] = posting_results
        st.session_state['show_posting_results'] = True
    ...
    if st.session_state.current_user_id:
        ...
        if client_id_env and client_secret_env:
            if st.button("🔐 Login with LinkedIn", type="primary", use_container_width=True):
                ...
                # Display authorization URL and button
                st.success("✅ Authorization URL generated successfully!")

                # Use st.link_button for proper external redirect
                st.link_button(
                    "🔐 Login with LinkedIn",
                    auth_url,
                    type="primary",
                ...
                st.caption("↑ Click the button above to authorize on LinkedIn. You'll be redirected back after authorization.")

                # Also show URL for manual copy if button doesn't work
                with st.expander("🔗 Alternative: Copy Authorization URL"):
                    st.code(auth_url, language=None)
                    st.caption("If the button doesn't work, copy this URL and paste it in your browser.")
        else:
